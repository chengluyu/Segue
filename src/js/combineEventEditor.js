var CombineEventEditor = {
  ...
  init: function () {
    ...
    // restore the title
    EventView.eventEditorSvg.select(".event-editor-text").text("Event Editor");
    ...
    self.paddingBetweenEvents =
      (EventView.eventEditorWidthMinusPadding -
        self.eventCombinationNextX * 2) /
      (self.maxEventCombination - 1);

    EventView.eventEditorSvg.append("g").attr("class", "editor");
  },
  updateControl: function (width, height, fill, name) {
    // add the symbol to event editor
    var self = this;

    if (self.numberOfEventCombined < self.maxEventCombination) {
      // draw symbol
      var dragCombinedEventSymbol = d3.behavior
        .drag()
        .on("dragstart", self.onDragStartSymbol)
        .on("drag", self.onDragSymbol)
        .on("dragend", self.onDragEndSymbol);

      EventView.eventEditorSvg
        .select(".editor")
        .append("rect")
  ...
  addEventToDatabase: function () {
    ...
    EventView.eventEditorSvg.selectAll(".combined-event").each(function () {
      eventsToBeCombined.push(d3.select(this).attr("name"));
    });
    EventView.eventEditorSvg
      .selectAll(".combined-event-interval")
      .each(function () {
    ...
    var eventsByNameAndEventName = d3
      .nest()
      .key(function (d) {
        return d.name;
      })
    ...
    for (var name in eventsByNameAndEventName) {
      for (var startDate in eventsByNameAndEventName[name][
        eventsToBeCombined[0]
      ]) {
        var currentAnchorRecord =
          eventsByNameAndEventName[name][eventsToBeCombined[0]][startDate][0]; // no overlapping, safe to use [0]
        ...
        var lastTimeStep =
          Database.networkDict[Database.nameList[0]][
        ...
        stopDate = new Date(
          stopDate.setMonth(stopDate.getMonth() + intervalsBetweenEvents[0])
        );
        ...
        if (checkingEvent in eventsByNameAndEventName[name])
          startDateListOfCheckingEvent = Object.keys(
            eventsByNameAndEventName[name][checkingEvent]
          );
        else break;
        ...
        while (currentDate <= lastDate) {
          ...
          if (
            $.inArray(currentDateString, startDateListOfCheckingEvent) != -1
          ) {
            // store the found combination if needed
            currentAnchorRecord =
              eventsByNameAndEventName[name][checkingEvent][
                currentDateString
              ][0]; // no overlapping, safe to use [0]
            ...
            stopDate = new Date(
              stopDate.setMonth(
                stopDate.getMonth() +
                  intervalsBetweenEvents[eventCombinationCount - 1]
              )
            );
  ...
  createDebugRectData: function () {
    ...
    EgoNetworkView.svgGroup.selectAll(".flow").each(function () {
      ...
      if (!(currentName in eventsByNameAndEventName)) return;

      for (var startDate in eventsByNameAndEventName[currentName][
        eventsToBeCombined[0]
      ]) {
        var currentAnchorRecord =
          eventsByNameAndEventName[currentName][eventsToBeCombined[0]][
            startDate
          ][0]; // no overlapping, safe to use [0]
        var currentAnchorRecordEndDate = currentAnchorRecord.endDate;
        var checkingEventIndex = 1;
        ...
        if (checkingEvent in eventsByNameAndEventName[currentName])
          startDateListOfCheckingEvent = Object.keys(
            eventsByNameAndEventName[currentName][checkingEvent]
          );
        else break;
  ...
  onDragStartSymbol: function () {
    ...
    d3.select(this).attr("original-x", originalX);
  },
  ...
  onDragEndSymbol: function () {
    ...
    if (y > -EventView.eventEditorStrokeWidth * 2) {
      // rectangle translate by -eventEditorStrokeWidth * 2
      ...
      // shift other symbols
      d3.selectAll(".combined-event").each(function () {
        var currentSymbolX = parseInt(d3.select(this).attr("x"));

        // for removing interval
        if (
          currentSymbolX < thisSymbolOriginalX &&
          currentSymbolX > previousSymbolOriginalX
        )
        ...
        if (currentSymbolX > maxX) maxX = currentSymbolX;
      });
      ...
      if (previousSymbolOriginalX != -999) {
        d3.selectAll(".combined-event-interval").each(function () {
          var currentIntervalX = parseInt(d3.select(this).attr("x"));

          if (
            currentIntervalX > previousSymbolOriginalX &&
            currentIntervalX < thisSymbolOriginalX
          ) {
            d3.select(this).remove();
      ...
      // restore parameters
      self.eventCombinationNextX =
        maxX + symbolWidth / 2 + self.paddingBetweenEvents;
      self.numberOfEventCombined--;

      // update the debug rect
      if (self.numberOfEventCombined == 0) EventView.removeEventEditor();

      if (self.numberOfEventCombined <= 1) {
        // nothing to combine
  ...
  onClickIntervalText: function () {
    ...
    EventView.createTextEdit(x, y, width, text, function (textInput) {
      ...
      if (month == 0 || month == 1) newText = "<= " + month + " month";
      else if (month > 1 && month <= 23) newText = "<= " + month + " months";
      else newText = d3.select(textNode).text();
