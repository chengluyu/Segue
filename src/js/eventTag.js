var EventTag = {
  ...
  onMouseEnter: function () {
    ...
    d3.select(this).select("rect").style("fill", "#FFFFEE");

    d3.select(this).selectAll(".instruction-text").style("display", null);
    ...
    EgoNetworkView.createDebugRect(
      self.debugEventsByName,
      self.debugEventColour
    );
  },
  createDebugRectData: function (selectedEventName) {
    ...
    for (var i = 0; i < Database.events.length; i++) {
      if (
        $.inArray(Database.events[i].name, selectedName) != -1 &&
        Database.events[i].eventName == selectedEventName
      ) {
        var currentName = Database.events[i].name;

        if (!(currentName in eventsByName))
          eventsByName[currentName] = [Database.events[i]];
        else eventsByName[currentName].push(Database.events[i]);
      }
    }
  ...
  onMouseLeave: function () {
    ...
    d3.select(this).selectAll(".instruction-text").style("display", "none");
  ...
  onDoubleClick: function () {
    ...
    var removedTagTranslateY = d3.transform(d3.select(this).attr("transform"))
      .translate[1];
    d3.select(this).remove();
    d3.selectAll(".event-tag").each(function () {
  ...
  onDragEnd: function () {
    ...
    var y = d3.mouse(this)[1];
    var targetY =
      EventView.eventPanelHeight -
      d3.transform(d3.select(this).attr("transform")).translate[1] -
      EventView.margin.top;
