var MDSView = {
  ...
  init: function () {
    ...
    self.width =
      $("#scatterplot").width() - self.margin.left - self.margin.right;
    self.height =
      $("#scatterplot").height() - self.margin.top - self.margin.bottom;
    ...
    d3.select("#scatterplot")
      ...
      .attr(
        "transform",
        "translate(" + self.textMargin.left + ", " + self.textMargin.top + ")"
      )
  ...
  initDistanceFunctionMenu: function () {
    ...
    $("#mds-view .distance-function-menu select").on(
      "change",
    ...
    function changeDistanceFunction() {
      var selectedDistance = $(
        "#mds-view .distance-function-menu select"
      ).val();
      var isHeatmapSelected = d3
        .select("#scatterplot .heatmap-btn")
        .classed("selected");
  ...
  initHeatmapButton: function () {
    ...
    heatmapButton
      ...
      .attr(
        "transform",
        "translate(" + networkViewWidth + ", " + self.textMargin.top + ")"
      )
      .style("text-anchor", "end")
      .text("Heatmap");
    var bbox = heatmapButton.select("text").node().getBBox();
    heatmapButton
      .insert("rect", ".heatmap-btn text")
      .attr("x", bbox.x - 2)
      .attr("y", bbox.y - 1)
  ...
  initClickOutside: function () {
    ...
    d3.select("#scatterplot .background").on("click", function () {
  ...
  initLabel: function () {
    ...
    self.labelSVG = d3
      ...
      .attr(
        "transform",
        "translate(" + self.margin.left + "," + labelTranslateY + ")"
      )
      .style("cursor", "pointer");
  ...
  highlightTimeline: function (timeIndex) {
    ...
    self.labelSVG.selectAll("circle").style("fill", "white");

    // text
    var targetText = self.labelSVG.selectAll(".date")[0][timeIndex];
    d3.select(targetText).style("font-size", 13).style("font-weight", "bold");

    // circle
    var targetCircle = self.labelSVG.selectAll("circle")[0][timeIndex];
    d3.select(targetCircle).style("fill", "#d3d3d3");
  },
  ...
  update: function () {
    ...
    if (Database.events.length == 0) self.restore();
    if (Database.events.length != 0 && selectedDistance == "euclidean") {
      ComparisonHandler.computeFeatureVectors();
      ComparisonHandler.computeScatterplotCoord();
      self.updateNodes(StateHandler.restoreState);
  ...
  updateNodes: function (callback) {
    ...
    self.nodeLayer.selectAll("circle").classed("clicked", false);

    // join
    var circle = self.nodeLayer
      .selectAll("circle")
    ...
    function dblClickCircle(clickedCircleData) {
      ...
      if (Database.events.length != 0) {
        for (var theOtherLabel in ComparisonHandler.featureVectors) {
          if (theOtherLabel != clickedCircleData.label) {
            var clickedFeatureVector =
              ComparisonHandler.featureVectors[clickedCircleData.label];
            var theOtherFeatureVector =
              ComparisonHandler.featureVectors[theOtherLabel];
            var distance = ComparisonHandler.computeVectorDistance(
              clickedFeatureVector,
              theOtherFeatureVector
            );
        ...
        if (this != clickedCircleEl) {
          var currentRadius =
            Database.events.length != 0
              ? radiusScale(labelToDistanceDict[d.label])
              : self.width / 2;
      ...
      if (isHeatmapSelected) Heatmap.show(allCoords);
    }
    ...
    function mouseoverCircle(d) {
      ...
      var pixelGroupWidth =
        Table.columnWidth[2] -
        Table.pixelGroupLeftRightPadding * 2 +
        newPixelGroupLeftRightPadding * 2;

      // * draw pixel display
      var pixelHeight =
        (Table.maxPixelGroupsHeight - (EventView.maxNumberOfEvents - 1)) /
        EventView.maxNumberOfEvents; // 6 = gap numbers x gap pixels
      var pixelWidth = 4;
      var paddingBetweenPixel =
        (Table.columnWidth[2] -
          Table.pixelGroupLeftRightPadding -
          Table.pixelGroupLeftRightPadding -
          pixelWidth * Database.numberOfTimeSteps) /
        (Database.numberOfTimeSteps - 1);
      var pixelXAndWidthScale = d3.scale
        .linear()
        .domain([0, Database.numberOfTimeSteps - 1])
        .range([
          0,
          pixelGroupWidth -
            newPixelGroupLeftRightPadding -
            newPixelGroupLeftRightPadding,
        ]);
      ...
      var currentEventPixelGroup = self.nodeLayer
        ...
        .attr(
          "transform",
          "translate(" + (circleX + 10) + ", " + (circleY - 30) + ")"
        );

      // append text label
      currentEventPixelGroup
        .append("text")
        .attr("x", 2)
        .attr("y", -5)
      ...
      var currentPixelGroup = currentEventPixelGroup
        .selectAll("g")
        ...
        .attr("transform", function (d, i) {
          var yTranslate =
            Table.maxPixelGroupsHeight - (i + 1) * (pixelHeight + 1);
          return (
            "translate(" +
            newPixelGroupLeftRightPadding +
            ", " +
      ...
      currentPixelGroup
        ...
        .attr("width", function (d) {
          ...
          if (d.endDate) {
            ...
            return (
              pixelXAndWidthScale(endTimeIndex - startTimeIndex) -
              paddingBetweenPixel
            );
          } else {
            return pixelWidth;
        ...
        .attr("x", function (d) {
          return (
            pixelXAndWidthScale(Database.dateString2Index[d.startDate]) +
            paddingBetweenPixel / 2
          );
        })
  ...
  updateLinks: function (date) {
    ...
    for (
      var i = 0;
      i < ComparisonHandler.scatterplotCoordForDisplay.length;
      i++
    ) {
    ...
    for (var i = 0; i < Database.dateToLinkDict[date].length; i++) {
      ...
      if (linkString in links) links[linkString]++;
      else links[linkString] = 1;
    }
    ...
    for (linkID in links) {
      ...
      var className =
        sourceLabel.split(".").join("-") +
        " " +
        targetLabel.split(".").join("-");
      ...
      if (sourceLabel != targetLabel) {
        var linkObject = {
          ...
          target: {
            x: labelCoord[targetLabel].x,
            y: labelCoord[targetLabel].y,
          },
        };
    ...
    var linkSVG = self.linkLayer.selectAll("path.link").data(linkData);
    ...
    // draw circle
    var linkSVG = self.linkLayer.selectAll("circle.link").data(circleData);
  ...
  highlightEgoNetwork: function (
    ...
    self.linkLayer.selectAll(".link").style("opacity", 0.05);
    self.nodeLayer.selectAll("circle").attr("r", 5).style("opacity", 0.05);

    // highlight nodes
    for (var i = 0; i < nodeClassNameList.length; i++) {
      self.nodeLayer
        .select("circle" + nodeClassNameList[i])
        .style("opacity", 0.7);
    }
    ...
    for (var i = 0; i < linkClassNameList.length; i++) {
      self.linkLayer
        .select(linkClassNameList[i] + ".link")
        .style("opacity", 0.7);
    }

    // highlight ego
    if (egoClassName != null)
      self.nodeLayer.select("circle" + egoClassName).attr("r", 10);
  },
  removeHighlightEgoNetwork: function () {
    var self = this;

    self.linkLayer.selectAll(".link").style("opacity", 0.7);
    self.nodeLayer.selectAll("circle").attr("r", 5).style("opacity", 0.7);
    self.nodeLayer.selectAll("circle.clicked").attr("r", 10);
  },
  restoreBackgroundPointerEvents: function () {
    d3.select("#scatterplot .background").style("pointer-events", null);
  },
  disableBackgroundPointerEvents: function () {
    d3.select("#scatterplot .background").style("pointer-events", "none");
  },
};
