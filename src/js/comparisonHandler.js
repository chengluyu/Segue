var ComparisonHandler = {
  ...
  computeEventArray: function () {
    ...
    var event2IndexDict = {};
    var eventsByNameAndStartDate = d3
      .nest()
      .key(function (d) {
        return d.name;
      })
      ...
      if (currentName in eventsByNameAndStartDate) {
        for (var j = 0; j < Database.dateStringArray.length; j++) {
          ...
          if (currentDate in eventsByNameAndStartDate[currentName]) {
            var eventsAtCurrentDate =
              eventsByNameAndStartDate[currentName][currentDate];
  ...
  computeScatterplotCoord_edit: function () {
    ...
    for (var label in self.eventArrayForEachEgo) labels.push(label);
    ...
    for (var i = 0; i < labels.length; i++) {
      ...
      for (var j = 0; j < labels.length; j++) {
        ...
        var distance = self.computeEditDistance(
          currentEventArray,
          theOtherEventArray
        );
    ...
    var xScale = d3.scale
      .linear()
      .domain(
        d3.extent(ComparisonHandler.scatterplotCoord, function (d) {
          return d.x;
        })
      )
  ...
  computeEditDistance: function (eventArray1, eventArray2) {
    ...
    for (var i = 0; i < eventArray1.length + 1; i++) {
      // +1 for empty char
      distanceMatrix.push([]);
    ...
    for (var i = 1; i < eventArray2.length + 1; i++) {
      // first row
    ...
    for (var i = 1; i < eventArray1.length + 1; i++) {
      for (var j = 1; j < eventArray2.length + 1; j++) {
        ...
        if (isSameCharacter) currentCost = diagValue;
        if (!isSameCharacter)
          currentCost =
            Math.min(topValue, Math.min(leftValue, diagValue)) + costOfTwoChar;
  ...
  computeFeatureVectors: function () {
    ...
    for (var i = 0; i < Database.nameList.length; i++) {
      ...
      var vector = [];
      for (var j = 0; j < allEvents.length; j++) vector.push(0);
      ...
      for (var j = 0; j < allEvents.length; j++) {
        ...
        if (
          currentName in eventsByNameAndEventName &&
          currentEvent in eventsByNameAndEventName[currentName]
        )
          vector[j] =
            eventsByNameAndEventName[currentName][currentEvent].length;
      }
  ...
  computeOriginalScatterplotCoord: function () {
    ...
    for (var i = 0; i < labels.length; i++) {
      ...
      for (var j = 0; j < labels.length; j++) {
        ...
        var distance = 1.5;
        if (currentLabel == theOtherLabel) distance = 0;
        else if (
          Database.employeeDict[currentLabel] ==
          Database.employeeDict[theOtherLabel]
        )
  ...
  computeScatterplotCoord: function () {
    ...
    // create a list of labels
    for (var label in self.featureVectors) labels.push(label);
    ...
    for (var i = 0; i < labels.length; i++) {
      ...
      for (var j = 0; j < labels.length; j++) {
        ...
        var distance = self.computeVectorDistance(
  ...
  jitterScatterplotCoordForDisplay: function () {
    ...
    for (var i = 0; i < self.scatterplotCoord.length; i++) {
      var newX =
        xScale(self.scatterplotCoord[i].x) +
        Math.floor(Math.random() * 20) -
        10;
      var newY =
        yScale(self.scatterplotCoord[i].y) +
        Math.floor(Math.random() * 20) -
        10;
