var EventView = {
  ...
  init: function () {
    ...
    self.eventEditorWidthMinusPadding =
      eventViewWidth - self.margin.left - self.margin.right;
    self.eventEditorHeightMinusPadding =
      self.eventEditorHeight - self.margin.top - self.margin.bottom;
    ...
    // right click
    EventViewRightClickHandler.installRightClickBehaviour(
      d3.select("#event-editor")
    );
    self.initMouseenterBehaviour();

    // event panel
    self.eventPanelSvg = d3
      .select("#event-editor")
      .append("g")
      .attr(
        "transform",
    ...
    var clearButton = self.eventPanelSvg
      .append("g")
    ...
    self.eventEditorSvg = d3
      ...
      .attr(
        ...
        "translate(" +
          self.margin.left +
          ", " +
          (self.eventPanelHeight + self.margin.top) +
          ")"
      );
    self.eventEditorSvg
      .append("line")
    ...
    self.eventEditorSvg
      ...
      .attr(
        "height",
        self.eventEditorHeightMinusPadding + self.eventEditorStrokeWidth * 2
      )
      .style("fill", "none")
      .style("stroke", "none")
    ...
    var clearButton = self.eventEditorSvg
      .append("g")
      .style("cursor", "pointer")
  ...
  addEventTagToEventPanel: function (drawCircle) {
    ...
    var eventGroup = self.eventPanelSvg
      .append("g")
      .attr("class", function () {
  ...
  checkIfCurrentEventAdded: function () {
    ...
    if (self.isAdded) {
      self.newEventColour = self.colours(
        self.event2Index[self.currentEventName]
      );
  ...
  updateEventEditor: function () {
    ...
    // update the text
    self.eventEditorSvg.select(".event-name").text(self.currentEventName);
    ...
    var addButton = self.eventEditorSvg
      .select(".add-button")
      .attr(
        "transform",
        "translate(" + self.addButtonXTranslate + ", " + self.tagY + ")"
      );
    var addButtonSvgText = addButton.select("text").text(self.addButtonText);
    var bbox = addButtonSvgText[0][0].getBBox();
    self.eventEditorSvg
      .select(".add-button rect")
  ...
  storeEventName: function (newEventName) {
    // for checking if the event name is duplicated
    var self = this;
    ...
    for (var i = 0; i < currentEventList.length; i++) {
      ...
      if (foundIndex != -1) {
        if (currentEventList[i].indexOf("#") == -1) {
          // not found
          maxEventID = 1;
        } else {
    ...
    else EventView.currentEventName = newEventName + " #" + (maxEventID + 1);
  ...
  createTextEdit: function (x, y, width, text, useTextInput) {
    ...
    var textEditDiv = self.eventEditorSvg
  ...
  updateNextEventIndex: function () {
    // should be called every time event2Index changes
    var self = this;

    // some events in the middle from 0 to 6 may be deleted, need to fill in the gap
    var isFilledArray = [];
    for (var i = 0; i < self.maxNumberOfEvents; i++) isFilledArray.push(false);
