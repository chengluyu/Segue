...
$(function () {
  ...
  $("#right-content").css("width", rightContentWidth).css("height", fullHeight);
  $("#right-content #upper-content")
    .css("width", full)
    .css("margin-bottom", margin);
  $("#right-content #lower-content").css("width", full);

  // left content
  $("#mds-view").css("width", full).css("height", full);
  $("#mds-view #heatmap").css("width", full).css("height", full);
  $("#mds-view .scatterplot-container").css("width", full).css("height", full);
  d3.select("#mds-view #scatterplot").attr("width", full).attr("height", full);

  // right upper contents
  $("#flow-view")
    .css("width", flowViewWidth)
    .css("height", flowSvgHeight + timelineSvgHeight + menuBarHeight);
  $("#flow-view .ui-menu-bar").css("width", full).css("height", menuBarHeight);
  ...
  $("#table-view").css("width", tableViewWidth).css("height", tableViewHeight);
