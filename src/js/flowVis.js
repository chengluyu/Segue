var FlowVis = {
  ...
  init: function (svg, width, height, name) {
    ...
    self.numberOfNodessAtEachTimeStep = self.returnNumberOfNodesAtEachTimeStep(
      name
    );
  },
  ...
  createLabels: function () {
    ...
    var data = [];
    for (var i = 0; i < Database.numberOfTimeSteps; i++) data.push(i);
    ...
    // create tick marks
    var tickGroup = self.allFlowSVG
      .append("g")
      .attr("class", "tick-group")
      .attr("transform", "translate(0," + yTranslate + ")");
    var tick = tickGroup.selectAll("g").data(data).enter().append("g");
  ...
  updateFocus: function (className, timeIndex) {
    ...
    var widthOfOneTimePeriod =
      (EgoNetworkView.canvasWidth -
        EgoNetworkView.margin.left -
        EgoNetworkView.margin.right) /
      totalNumberOfTimePeriods;

    var height = flowGroup.select(".flow-vis").node().getBBox().height;
    var firstStartX = 0;
    var firstEndX =
      (timeIndex - 1) * widthOfOneTimePeriod + widthOfOneTimePeriod / 2;
    var secondStartX =
      (timeIndex + 1) * widthOfOneTimePeriod - widthOfOneTimePeriod / 2;
    var secondEndX = (totalNumberOfTimePeriods + 1) * widthOfOneTimePeriod;

    if (flowGroup.select(".flow-vis").select(".start-focus").empty()) {
      flowGroup.select(".flow-vis").append("rect").attr("class", "start-focus");
    }

    if (flowGroup.select(".flow-vis").select(".end-focus").empty()) {
      flowGroup.select(".flow-vis").append("rect").attr("class", "end-focus");
    }

    flowGroup
      .select(".flow-vis .start-focus")
      .attr("x", firstStartX)
      .attr("y", -height / 2)
  ...
  firstLayer: {
      ...
      var area = d3.svg
        ...
        .y0(function (d, i) {
          ...
          if (self.numberOfNodessAtEachTimeStep[i] > 0)
            paddingForNodes =
              2 * self.gapBetweenNodes +
              (self.numberOfNodessAtEachTimeStep[i] - 1) * self.gapBetweenNodes;
    ...
    translateFlow: function () {
      ...
      self.thisFlowSVG.attr(
        "transform",
        "translate(0, " + flowBBox.height / 2 + ")"
      );
    },
  ...
  secondLayer: {
    ...
    createNodeArray: function () {
      ...
      for (var i = 0; i < self.attributeData.length; i++) {
        ...
        if (currentNumberOfNodes > 0)
          paddingForNodes =
            2 * self.gapBetweenNodes +
            (currentNumberOfNodes - 1) * self.gapBetweenNodes;

        var widthOfFlow =
          flowSizeScale(self.networkData[i].size) + paddingForNodes;

        // should have padding between, top and bottom
        var widthOfFlowMinusGap =
          widthOfFlow -
          (currentNumberOfNodes * self.gapBetweenNodes + self.gapBetweenNodes);
        ...
        for (var j = 0; j < self.attributeData[i].length; j++) {
          // loop through all clusters at a time step
          var nodeHeight = sizeScale(self.attributeData[i][j].frequency);
          var x = xScale(i);
          y += self.gapBetweenNodes + nodeHeight / 2;
          var positionIndex =
            Database.position2Index[self.attributeData[i][j].position];
    ...
    createLinkArray: function () {
      ...
      for (
        var i = 0;
        i < self.secondLayer.nodesForEachTimeStep.length - 1;
        i++
      ) {
        for (
          var j = 0;
          j < self.secondLayer.nodesForEachTimeStep[i].length;
          j++
        ) {
          ...
          for (
            var k = 0;
            k < self.secondLayer.nodesForEachTimeStep[i + 1].length;
            k++
          ) {
            var nodeInNextTimeStep =
              self.secondLayer.nodesForEachTimeStep[i + 1][k];
    ...
    createLinks: function (hideLinks) {
      ...
      if (hideLinks) self.thisFlowSVG.selectAll(".link").style("opacity", 0);
    },
