var IntervalEventEditor = {
  ...
  init: function (attribute) {
    ...
    self.sliderFirstColumnWidth =
      ((eventViewWidth - EventView.margin.left - EventView.margin.right) / 3) *
      2;
    self.sliderWidth["Slope"] =
      self.sliderFirstColumnWidth -
      self.sliderLeftPadding -
      self.sliderRightPadding;
    self.sliderWidth["Time period"] =
      self.sliderFirstColumnWidth / 2 -
      self.sliderLeftPadding -
      self.sliderRightPadding;
    self.sliderWidth["Noise"] =
      self.sliderFirstColumnWidth / 2 -
      self.sliderLeftPadding -
      self.sliderRightPadding;

    self.editorSVG = EventView.eventEditorSvg
      .append("g")
      .attr("class", "editor")
      .attr(
        "transform",
        "translate(" + self.sliderLeftPadding + "," + EventView.brushY + ")"
      );
  },
  ...
  createEventTagInEditor: function () {
    ...
    // create group
    var editorEventTag = EventView.eventEditorSvg
      .append("g")
    ...
    // draw add button
    var addButton = editorEventTag
      .append("g")
      .attr("class", "add-button")
      .attr(
        "transform",
        "translate(" +
          EventView.addButtonXTranslate +
          ", " +
          EventView.tagY +
          ")"
      )
    ...
    var addButtonSvgText = addButton
      .append("text")
      .attr("x", 0)
      .attr("y", 0)
  ...
  brushCreator: {
    createSlopeBrush: function () {
      ...
      var trueStartValue =
        Math.round(
          Database.slopeRangeDict[self.currentAttribute][0] * 1.5 * 10
        ) / 10;
      var trueEndValue =
        Math.round(
          Database.slopeRangeDict[self.currentAttribute][1] * 1.5 * 10
        ) / 10;
      ...
      self.brush[name] = self.brushCreator.createBrush(
        name,
        x,
        -3,
        startValue,
      ...
      self.brush[name]
        .on("brush", function () {
          self.brushCreator.onBrushMove(this);
        })
        .on("brushend", function () {
          self.brushCreator.onBrushEnd(this);
        });
      ...
      self.editorSVG
        .select("." + className + ".brush .overall-lower-bound")
        .attr("x", xScale(trueStartValue))
        .text(trueStartValue);
      self.editorSVG
        .select("." + className + ".brush .overall-upper-bound")
        .attr("x", xScale(trueEndValue))
        .text(trueEndValue);

      self.editorSVG
        .select("." + className + ".brush")
        .append("line")
    ...
    createNoiseBrush: function () {
      ...
      self.editorSVG
        .select(".brush." + className + " .extent")
        .attr("width", self.brush[name].x()(0.03));
      self.editorSVG
        .selectAll("." + className + ".brush .resize")
        .style("display", null);
      self.editorSVG
        .select("." + className + ".brush .resize.e")
        .attr("transform", "translate(" + self.brush[name].x()(0.03) + ",0)");
      self.editorSVG
        .select("." + className + ".brush .brush-lower-bound")
        .text(self.brush[name].extent()[0]);
      self.editorSVG
        .select("." + className + ".brush .brush-upper-bound")
    ...
    createBrush: function (
      ...
      var brushGroup = self.editorSVG
        ...
        .attr(
          "transform",
          "translate(" +
            (xColumnShift + self.sliderLeftPadding) +
            ", " +
            yTranslate +
            ")"
        )
      ...
      brushGroup.selectAll("rect").attr("height", self.sliderHeight[name]);
      brushGroup.select(".background").style("visibility", null);
    ...
    onBrushMove: function (brushSVG) {
      ...
      if (self.isInt[name]) {
        lowerBound = self.brush[name].empty()
          ? null
          : Math.round(self.brush[name].extent()[0]);
        upperBound = self.brush[name].empty()
          ? null
          : Math.round(self.brush[name].extent()[1]);
      } else {
        // show two decimal place
        lowerBound = self.brush[name].empty()
          ? null
          : Math.round(self.brush[name].extent()[0] * 100) / 100;
        upperBound = self.brush[name].empty()
          ? null
          : Math.round(self.brush[name].extent()[1] * 100) / 100;
      }
      ...
      if (name == "Slope") {
        ...
        if (brushLowerBound < lowerThreshold) {
          self.editorSVG.select(".Slope.brush .brush-lower-bound").text("-∞");
        }
        if (brushUpperBound > upperThreshold) {
          self.editorSVG.select(".Slope.brush .brush-upper-bound").text("∞");
        }
    ...
    onBrushEnd: function (brushSVG) {
      ...
      if (self.brush["Slope"].empty()) {
        EventView.eventEditorSvg.select(".add-button").style("display", "none");
      } else {
        EventView.eventEditorSvg.select(".add-button").style("display", null);
      }
  ...
  clickAddButton: function () {
    ...
    // update the name
    var originalName = EventView.eventEditorSvg
      .select("text.event-name")
      .text();
  ...
  addEventToDatabase: function () {
    ...
    for (var name in Database.timeSeriesRegressionDict[self.currentAttribute]) {
      var slopeArray =
        Database.timeSeriesRegressionDict[self.currentAttribute][name].slope;
      var outlierPercentArray =
        Database.timeSeriesRegressionDict[self.currentAttribute][name]
          .outlierPercent;
      ...
      for (var i = 0; i < maxIndex; i++) {
        ...
        var outOfRange =
          secondIndex - firstIndex < timePeriodRange[0] ||
          secondIndex - firstIndex >= timePeriodRange[1]; // upper bound not included
        if (outOfRange) {
          firstIndex++;
        ...
        if (isMatch) endPointOfLongestMatch = secondIndex;

        // store if it is the last item in the loop
        var lastItemInLoop =
          secondIndex == Database.numberOfTimeSteps - 1 ||
          secondIndex == firstIndex + timePeriodRange[1];
        if (lastItemInLoop && endPointOfLongestMatch) {
          Database.appendEvent(
            EventView.currentEventName,
            Database.dateStringArray[firstIndex],
            Database.dateStringArray[endPointOfLongestMatch],
            name
          );
  ...
  getSlopeRange: function () {
    ...
    var slopeRange = self.brush["Slope"].empty()
      ? null
      : self.brush["Slope"].extent();
    ...
    if (slopeRange) {
      ...
      slopeRange[0] =
        slopeRange[0] < lowerThreshold
          ? -Infinity
          : Math.round(slopeRange[0] * 100) / 100;
      slopeRange[1] =
        slopeRange[1] > upperThreshold
          ? Infinity
          : Math.round(slopeRange[1] * 100) / 100;
  ...
  getTimePeriodRange: function () {
    ...
    var timePeriodRange = self.brush["Time period"].empty()
      ? null
      : self.brush["Time period"].extent();
  ...
  getNoiseRange: function () {
    ...
    var noiseRange = self.brush["Noise"].empty()
      ? null
      : self.brush["Noise"].extent();
  ...
  createDebugRectData: function () {
    ...
    EgoNetworkView.svgGroup.selectAll(".flow").each(function () {
      ...
      // adding new event
      var slopeArray =
        Database.timeSeriesRegressionDict[self.currentAttribute][currentName]
          .slope;
      var outlierPercentArray =
        Database.timeSeriesRegressionDict[self.currentAttribute][currentName]
          .outlierPercent;
      ...
      for (var i = 0; i < maxIndex; i++) {
        ...
        // check if slope falls into range
        var slope = slopeArray[i];
        if (slope < slopeRange[0] || slope >= slopeRange[1]) isMatch = false;
